    ...
    def _extract_query_type(self, schema):
        ...
        if "queryType" in schema:
            ...
            if name:
                ...
                raise Exception(
                    "GQLSchema: Invalid schema - queryType is null. File a bug if this happens in real "
                    "world."
                )
        else:
            raise Exception(
                "GQLSchema: Invalid schema - no queryType (absence of queryType is not allowed according "
