...
@click.option(
    ...
    "--list",
    "stuff_to_print",
    help="Parse GraphQL schema and list queries, mutations or both of them (valid "
    "values are: 'queries', 'mutations' or 'all').",
)
@click.option(
    "-q",
    "--query",
    help="Only print named queries (argument is a comma-separated list of query names).",
)
@click.option(
    "-m",
    "--mutation",
    help="Only print named mutations (argument is a comma-separated list of mutation names).",
)
@click.option(
    "-Q",
    "--all-queries",
    is_flag=True,
    help="Only print queries (by default both queries and mutations are printed).",
)
@click.option(
    "-M",
    "--all-mutations",
    is_flag=True,
    help="Only print mutations (by default both queries and mutations are " "printed).",
)
...
def run(file_, url, all_queries, all_mutations, query, mutation, stuff_to_print):
    ...
    if schema.mutation and (all_mutations or mutation):
        if all_mutations:
            ...
            mutations_to_print = (
                schema.mutation.fields[m] for m in mutation.split(",")
            )
